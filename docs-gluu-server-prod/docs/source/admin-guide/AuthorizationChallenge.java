...
import org.gluu.oxauth.authorize.ws.rs.AuthorizationChallengeSessionService;
...
public class AuthorizationChallenge implements AuthorizationChallengeType {
     ...
     *
     * @param scriptContext ExternalScriptContext
     * @return true if Authorization Challenge Endpoint should return code successfully or otherwise false if error should be returned.
     */
    @Override
    public boolean authorize(Object scriptContext) {
        ...
        //    Note: prepareSession method implemention
        ...
        boolean isUserActive = StringUtils.equals(user.getStatus(), "ACTIVE");
        if (!isUserActive){
    ...
    private String getParameterOrCreateError(ExternalScriptContext context, String parameterName) {
        ...
        if (StringUtils.isBlank(value)) {
            ...
            value = getParameterFromSession(context, parameterName);
        }
    ...
    private void createError(ExternalScriptContext context, String errorCode) {
        String sessionPart = prepareSessionSubJson(context);

        final String entity = String.format("{\"error\": \"%s\"%s}", errorCode, sessionPart);
        context.createWebApplicationException(401, entity);
    }

    private String prepareSessionSubJson(ExternalScriptContext context) {
        AuthorizationChallengeSession sessionObject = context.getExecutionContext().getAuthzRequest().getAuthorizationChallengeSessionObject();
        if (sessionObject != null) {
            prepareSession(context, sessionObject);
            return String.format(",\"auth_session\":\"%s\"", sessionObject.getId());
        } else if (context.getExecutionContext().getAuthzRequest().isUseAuthorizationChallengeSession()) {
            sessionObject = prepareSession(context, null);
            return String.format(",\"auth_session\":\"%s\"", sessionObject.getId());
        }
        return "";
    }

    private AuthorizationChallengeSession prepareSession(ExternalScriptContext context, AuthorizationChallengeSession sessionObject) {
        AuthorizationChallengeSessionService authzSessionService = CdiUtil.bean(AuthorizationChallengeSessionService.class);
    ...
    private String getParameterFromSession(ExternalScriptContext context, String parameterName) {
        final AuthorizationChallengeSession sessionObject = context.getExecutionContext().getAuthzRequest().getAuthorizationChallengeSessionObject();
        if (sessionObject != null) {
